...
const AdminCodingQuestions = () => {
  ...
  const fetchStats = async () => {
    try {
      const response = await fetch('http://localhost:8081/api/coding-questions/stats')
      if (response.ok) {
        const data = await response.json()
        setStats({
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      const url = editingQuestion
        ? `http://localhost:8081/api/coding-questions/update/${editingQuestion.id}`
        : 'http://localhost:8081/api/coding-questions/create'
  ...
  const handleDelete = async (id) => {
    if (window.confirm('Are you sure you want to delete this question? This action cannot be undone.')) {
      try {
        const response = await fetch(`http://localhost:8081/api/coding-questions/delete/${id}`, {
          method: 'DELETE'
        })
