...
import com.codeverge.talentportal.service.CodingResultService;
...
public class CodingQuestionController {
    ...
    @Autowired
    private CodingResultService codingResultService;
    ...
    // Submit coding results
    @PostMapping("/submit-results")
    public ResponseEntity<?> submitResults(@RequestBody List<CodingResult> results) {
        try {
            List<CodingResult> savedResults = codingResultService.saveAllResults(results);
            return ResponseEntity.ok(Map.of(
                "success", true,
                "message", "Coding results saved successfully",
                "count", savedResults.size()
            ));
        } catch (Exception e) {
            return ResponseEntity.badRequest().body(Map.of(
                "success", false,
                "message", "Failed to save coding results: " + e.getMessage()
            ));
        }
    }
